...
export async function commandSlides(lesson: CommandLesson, deck: DeckBlock | null, signal?: AbortSignal): Promise<CommandSlide[]> {
  ...
  const url = new URL(deck.src, window.location.origin)
  ...
  const slides = [...doc.querySelectorAll('deck-stage > section')].map((slide, i) => ({ label: slide.getAttribute('data-label') || `Slide ${i + 1}`, notes: slide.getAttribute('data-speaker-notes') || '', anchor: slide.getAttribute('data-section-anchor') }))
