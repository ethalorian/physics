...
import proj1 from '@/data/proj-1-lesson-plans.json'
import proj2 from '@/data/proj-2-lesson-plans.json'
import proj3 from '@/data/proj-3-lesson-plans.json'
import proj4 from '@/data/proj-4-lesson-plans.json'
...
export const PLANS: Record<string, Record<string, DayPlan[]>> = {
  ...
  // Project Physics (program = 'projects', the MVP CPA section). One unit per
  // mission phase; two grains inside it — a week overview at day = week*10 and,
  // where they exist, day plans at day = week*10 + n. Generated by
  // scripts/gen-projects-lesson-plans.py; do not hand-edit the JSON.
  projects: {
    'proj-1': proj1 as DayPlan[],
    'proj-2': proj2 as DayPlan[],
    'proj-3': proj3 as DayPlan[],
    'proj-4': proj4 as DayPlan[],
  },
}
...
export const UNIT_LABEL: Record<string, string> = {
  ...
  'proj-1': 'Phase 1 · Track it — Motion, Forces & Structures',
  'proj-2': 'Phase 2 · Move it — Energy',
  'proj-3': 'Phase 3 · Sense it — Fluids, Waves & Electromagnetism',
  'proj-4': 'Phase 4 · Power it, prove it — Generation & the Capstone',
}

/** Curriculum plan set a course contributes. A course's `program` wins over its
 *  (physics) `track`: trades classes read the trades plans, Project Physics (the
 *  MVP section) reads the four mission phases, everything else reads its track. */
export function trackForCourse(c: { track: string | null; program: string | null }): string | null {
  if (c.program === 'trades') return 'trades'
  if (c.program === 'projects') return 'projects'
  return c.track
}
...
export async function resolveTracks(opts: { role: string; scopeEmail: string }): Promise<string[]> {
  ...
  return [...new Set(rows.map(trackForCourse).filter((t): t is string => Boolean(t)))]
}
