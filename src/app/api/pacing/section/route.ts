...
// POST /api/pacing/section { course_id, program?, block?, current_unit_id?, unit_start_date?, current_lesson_id? }
//
// UNIT-LEVEL ONLY. A section is placed by (unit, unit start date, current
// lesson). Elapsed days are this block's meetings since the unit started when
// the school rotation is configured, else school weekdays. No-school dates come
// from the school-wide rotation calendar — the single source.

type CourseRow = { id: string; teacher_email: string | null }
type ScheduleRow = { block: string | null }
...
export const GET = withAuth(async (request, ctx) => {
    ...
    const [items, units, { data: schedRow }, { data: pacingRow }, gids, cal] = await Promise.all([
      ...
      loadUnits(program),
      supabaseAdmin.from('section_schedules').select('block').eq('course_id', courseId).maybeSingle(),
    ...
    const block = ((schedRow as ScheduleRow | null)?.block as Block | null) ?? null
    ...
    if (rotationOn && block && ui.length > 0) {
...
export const POST = withAuth(async (request, ctx) => {
    ...
    if (body.block !== undefined) {
      ...
      if (block && !['A', 'B', 'C', 'D', 'E', 'F', 'G'].includes(block)) return NextResponse.json({ error: 'Block must be A–G' }, { status: 400 })
      const { error: sErr } = await supabaseAdmin.from('section_schedules')
        .upsert({ course_id: body.course_id, block, updated_at: new Date().toISOString() }, { onConflict: 'course_id' })
