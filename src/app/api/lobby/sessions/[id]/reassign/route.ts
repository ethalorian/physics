...
export const POST = withRole(['teacher', 'admin'], async (request, ctx) => {
  ...
  if (!user_id || !to_group_id) {
    return NextResponse.json({ error: 'user_id and to_group_id required' }, { status: 400 })
  }
  ...
  const { data: destGroup } = await supabaseAdmin
    .from('lobby_groups').select('id').eq('id', to_group_id).eq('session_id', id).maybeSingle()
  ...
  if (fromGroupId === to_group_id) return NextResponse.json({ ok: true, unchanged: true })

  // Move.
  await supabaseAdmin
    .from('lobby_members').update({ group_id: to_group_id }).eq('session_id', id).eq('user_id', user_id)

  // Re-issue passphrases for each affected group (delete emptied groups).
  const affected = [fromGroupId, to_group_id].filter((g): g is string => !!g)
