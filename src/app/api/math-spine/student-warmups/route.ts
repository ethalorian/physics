...
export const GET = withAuth(async (request, ctx) => {
  ...
  const query = () => {
    ...
    if (params.get('competency_id')) q = q.eq('competency_id', params.get('competency_id')!)
    return q
  }
  const [pending, recent, feedback, revisions, requested] = await Promise.all([
    ...
    supabaseAdmin.from('math_warmup_revisions').select('id,submission_id,response_json,message,needs_help,status,submitted_at,teacher_reply').eq('user_id', target.userId).order('submitted_at', { ascending: false }).limit(100),
