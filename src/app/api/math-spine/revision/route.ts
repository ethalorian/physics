...
export const POST = withAuth(async (request, ctx) => {
  ...
  if (body.action === 'acknowledge') {
    ...
    const saved = await supabaseAdmin.from('math_warmup_revisions').update({ status: 'acknowledged', teacher_reply: message, acknowledged_by: ctx.email, acknowledged_at: new Date().toISOString() }).eq('submission_id', sub.id).eq('status', 'pending').select('id')
  ...
  if (sub.status !== 'reviewed' || !sub.revision_requested) return NextResponse.json({ error: 'Your teacher has not requested a response to this work.' }, { status: 409 })
