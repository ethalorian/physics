...
export default function ProjectedBlockPage() {
  ...
  return <div data-projected-block-ready={shown && !error ? `${blockId}:${part}` : undefined}>
    <TeachingStage lesson={lesson?.title ?? 'Today’s lesson'} mode={blockMode(shown)} footer={<>{parts.length > 1 ? `Screen ${Math.min(part + 1, parts.length)} of ${parts.length} · ` : ''}Follow along on your device</>}>
      {error ? <p role="alert">{error}</p> : !lesson ? <p>Loading lesson block…</p> : !shown ? <p>This block is unavailable for the selected class.</p> : <TeachingBlockContent key={shown.id + ':' + part} block={shown} lessonId={lesson.id} referenceBlocks={lesson.content_blocks.blocks} />}
    </TeachingStage>
  </div>
}
