...
// Admin-only: this is where curriculum is BUILT, not taught.
    ...
    <div className="max-w-6xl mx-auto p-5" style={{ color: 'var(--foreground)' }}>
      ...
      <div className="flex items-center gap-3 flex-wrap mb-4">
        ...
        <div className="ml-auto flex items-center gap-2.5">
          ...
          <Link href="/admin/dashboard" className="text-sm font-semibold rounded-lg px-3.5 py-2" style={{ background: 'var(--primary)', color: 'var(--primary-foreground)' }}>+ New lesson</Link>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-2.5">
          {shelf?.gap && (
            <div className="flex items-center gap-3 rounded-xl px-4 py-3" style={{ border: '1px solid color-mix(in oklch, var(--reward) 45%, var(--border))', background: 'color-mix(in oklch, var(--reward) 7%, transparent)' }}>
              <span className="text-sm flex-1"><b>Next gap:</b> {shelf.gap.unitName} has fewer published lessons than targets — {shelf.gap.missing} short. Draft before your fastest section arrives.</span>
              ...
              <div className="flex flex-col gap-2">
                {unit.lessons.map((l) => (
                  <div key={l.id} className="flex items-center gap-3 rounded-xl border px-4 py-3" style={{ borderColor: l.published ? 'var(--border)' : 'color-mix(in oklch, var(--primary) 40%, var(--border))', background: l.published ? 'var(--card)' : 'color-mix(in oklch, var(--primary) 4%, var(--card))' }}>
                    ...
                    <Link href={`/admin/lessons/${l.id}`} className="text-xs font-semibold shrink-0" style={{ color: 'var(--primary)' }}>Edit</Link>
                  </div>
                ))}
