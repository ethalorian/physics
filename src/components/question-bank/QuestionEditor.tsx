...
import { Question, QuestionType, MultipleChoiceQuestion, EssayQuestion, NumericalQuestion, OpenResponseQuestion } from '@/types/assignment'
...
import { Trash2, Plus, Wand2, RefreshCw, Image as ImageIcon, Calculator, Save, X } from 'lucide-react'
import { Label } from '@/components/ui/label'
import RubricBuilder from '../assignment-builder/rubric-builder'
...
export default function QuestionBankQuestionEditor({ onClose, onSave }: QuestionEditorProps) {
  const { units } = useQuestionBank()
  ...
  const updateQuestion = (updates: Partial<Question>) => {
    setQuestion(prev => ({ ...prev, ...updates } as Question))
  }
      ...
      case 'open-response':
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-2">Correct Concepts</label>
                <Textarea
                  value={openResponseQuestion.correctConcepts?.join('\n') || ''}
                  onChange={(e) => updateQuestion({ 
                    correctConcepts: e.target.value.split('\n').filter(c => c.trim()) 
                  })}
                  placeholder="List key concepts (one per line)..."
                  rows={3}
                />
              </div>
              <div>
                ...
                <Textarea
                  ...
                  placeholder="List common misconceptions (one per line)..."
                  rows={3}
                />
              </div>
            </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {!question.scenarioImage && (
                    <Button
                      ...
                      className="bg-gradient-to-r from-emerald-50 to-teal-50 hover:from-emerald-100 hover:to-teal-100 border-emerald-200"
                    >
                      {isGeneratingImage ? (
                        <RefreshCw className="h-4 w-4 mr-2 animate-spin text-emerald-600" />
                      ) : (
