...
export default function TeachingTools({ live, state, busy, action, patch }: Props) {
  ...
  async function bookmark(text: string) {
    if (await action({ action: 'bookmark', note: text, student_id: studentId || null })) { setNote(''); setNotice('Private note saved to your teaching notes.') }
  }
  ...
  return <div className="space-y-4">
    <div className="grid items-start gap-4 xl:grid-cols-2">
      <Card id="help-queue" className="min-w-0 gap-3 p-4 scroll-mt-44"><h2 className="text-title-2">Who needs me?</h2><p className="text-caption text-muted-foreground">Private to you. Missing responses are check-in prompts, not evidence of low mastery.</p>
      ...
      <Card id="pulse-checks" className="min-w-0 gap-3 p-4 scroll-mt-44"><h2 className="text-title-2">Quick pulse check</h2><label className="flex min-h-12 items-center gap-3"><input type="checkbox" checked={anonymous} disabled={busy || state.pulse?.status === 'open'} onChange={e => setAnonymous(e.target.checked)} className="h-6 w-6" />Anonymous totals only</label><p className="text-caption text-muted-foreground">Anonymous checks never add names or missing-response flags to the help queue.</p>
