...
export default function WarmupAnswer({ needsGraph = false, needsEquationBuilder = false, value, onChange }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {needsEquationBuilder && (
        <div>
          <span className="text-sm font-semibold text-foreground">Build your equation</span>
          <p className="text-xs text-muted-foreground mb-2">Type your rearrangement / substitution step by step.</p>
          <EquationSandbox embedded value={sandbox} onChange={(v) => { setSandbox(v); emit({ sandbox: v }) }} />
        </div>
      )}

      <div>
        <span className="text-sm font-semibold text-foreground">{needsGraph ? 'Graph & show your work' : 'Show your work'}</span>
