...
(() => {
  ...
  const stylesheet = `
    ...
    .timer-chip[data-on] { display: inline-flex; }
    .timer-chip[data-warn] { color: #f2b24a; }
    .timer-chip[data-paused] { opacity: .65; }
    .timer-chip[data-done] { background: #b5563a; animation: deckTimerPulse 1s ease-in-out infinite; }
    @keyframes deckTimerPulse { 0%,100% { transform: scale(1); } 50% { transform: scale(1.06); } }
    .timer-chip .t-ic { display: inline-flex; opacity: .7; }
    .timer-chip .t-ic svg { width: 22px; height: 22px; }
    ...
    .timer-panel[data-open] { display: flex; }
    .timer-panel .t-label { opacity: .55; letter-spacing: .1em; font-size: 10px; padding-left: 2px; }
    .timer-row { display: flex; gap: 6px; align-items: center; }
    .timer-panel button {
      appearance: none;
      -webkit-appearance: none;
      border: 0;
    ...
    .timer-panel button:hover { background: rgba(255,255,255,.26); }
    .timer-panel button.t-start { background: #2f7d4f; }
    .timer-panel button.t-start:hover { background: #38955e; }
    .timer-panel input {
      width: 58px;
      background: rgba(255,255,255,.12);
      border: 0;
    ...
    .timer-panel input:focus { outline: 1px solid rgba(255,255,255,.5); }
    ...
    @media print {
      ...
      .overlay, .rail, .rail-resize, .ctxmenu, .confirm-backdrop, .timer-chip, .timer-panel { display: none !important; }
    }
  `;
  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <span class="divider"></span>
        <button class="btn timer-toggle" type="button" aria-label="Classroom timer" title="Timer (T)">Timer<span class="kbd">T</span></button>
      `;

      overlay.querySelector('.prev').addEventListener('click', () => this._advance(-1, 'click'));
      overlay.querySelector('.next').addEventListener('click', () => this._advance(1, 'click'));
      overlay.querySelector('.reset').addEventListener('click', () => this._go(0, 'click'));
      overlay.querySelector('.timer-toggle').addEventListener('click', () => this._timerTogglePanel());
      ...
      // Classroom timer: chip (student-visible, top-right) + teacher panel.
      const timerChip = document.createElement('div');
      timerChip.className = 'timer-chip export-hidden';
      timerChip.setAttribute('data-omelette-chrome', '');
      timerChip.title = 'Timer — click to adjust (T)';
      timerChip.innerHTML = `
        <span class="t-ic"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" aria-hidden="true"><circle cx="8" cy="9" r="5.5"/><path d="M8 6.5V9l2 1.5M6.5 1.5h3"/></svg></span>
        <span class="t-txt">0:00</span>`;
      timerChip.addEventListener('click', () => {
        if (this._timerDone) this._timerClear();
        else this._timerTogglePanel();
      });

      const timerPanel = document.createElement('div');
      timerPanel.className = 'timer-panel export-hidden';
      timerPanel.setAttribute('data-omelette-chrome', '');
      timerPanel.setAttribute('role', 'dialog');
      timerPanel.setAttribute('aria-label', 'Classroom timer');
      timerPanel.innerHTML = `
        <span class="t-label">CLASSROOM TIMER</span>
        <div class="timer-row">
          <button type="button" data-min="1">1 min</button>
          <button type="button" data-min="2">2 min</button>
          <button type="button" data-min="5">5 min</button>
          <button type="button" data-min="10">10 min</button>
        </div>
        <div class="timer-row">
          <input class="t-custom" type="number" min="1" max="120" step="1" placeholder="min" aria-label="Custom minutes">
          <button type="button" class="t-start">Start</button>
          <button type="button" class="t-pause">Pause</button>
          <button type="button" class="t-clear">Clear</button>
        </div>`;
      timerPanel.querySelectorAll('[data-min]').forEach((b) => {
        b.addEventListener('click', () => this._timerSet(parseInt(b.dataset.min, 10) * 60));
      });
      timerPanel.querySelector('.t-start').addEventListener('click', () => {
        const v = parseFloat(timerPanel.querySelector('.t-custom').value);
        if (v > 0) this._timerSet(Math.round(v * 60));
        else if (this._timerLeft > 0) this._timerStart();
      });
      timerPanel.querySelector('.t-pause').addEventListener('click', () => this._timerPause());
      timerPanel.querySelector('.t-clear').addEventListener('click', () => this._timerClear());
      this._root.append(timerChip, timerPanel);
      this._timerChip = timerChip;
      this._timerPanel = timerPanel;
      this._timerTxt = timerChip.querySelector('.t-txt');
      this._timerLeft = 0;
    ...
    _timerTogglePanel(force) {
      if (!this._timerPanel) return;
      const open = force != null ? force : !this._timerPanel.hasAttribute('data-open');
      if (open) this._timerPanel.setAttribute('data-open', '');
      else this._timerPanel.removeAttribute('data-open');
    }

    _timerSet(seconds) {
      this._timerLeft = seconds;
      this._timerDone = false;
      this._timerChip.removeAttribute('data-done');
      this._timerStart();
      this._timerTogglePanel(false);
    }

    _timerStart() {
      if (this._timerLeft <= 0) return;
      this._timerRunning = true;
      this._timerDone = false;
      this._timerDeadline = Date.now() + this._timerLeft * 1000;
      this._timerChip.setAttribute('data-on', '');
      this._timerChip.removeAttribute('data-paused');
      this._timerChip.removeAttribute('data-done');
      if (this._timerInterval) clearInterval(this._timerInterval);
      this._timerInterval = setInterval(() => this._timerTick(), 250);
      this._timerTick();
    }

    _timerPause() {
      if (!this._timerRunning) { this._timerStart(); return; }
      this._timerRunning = false;
      if (this._timerInterval) clearInterval(this._timerInterval);
      this._timerLeft = Math.max(0, Math.ceil((this._timerDeadline - Date.now()) / 1000));
      this._timerChip.setAttribute('data-paused', '');
    }
    ...
    _timerClear() {
      ...
      this._timerLeft = 0;
      if (this._timerInterval) clearInterval(this._timerInterval);
      this._timerChip.removeAttribute('data-on');
      this._timerChip.removeAttribute('data-done');
      this._timerChip.removeAttribute('data-warn');
      this._timerChip.removeAttribute('data-paused');
      this._timerTogglePanel(false);
    }

    _timerTick() {
      const left = Math.max(0, Math.ceil((this._timerDeadline - Date.now()) / 1000));
      const m = Math.floor(left / 60), s = left % 60;
      this._timerTxt.textContent = left > 0 ? `${m}:${String(s).padStart(2, '0')}` : 'TIME';
      if (left <= 10 && left > 0) this._timerChip.setAttribute('data-warn', '');
      else this._timerChip.removeAttribute('data-warn');
      if (left <= 0 && !this._timerDone) {
        this._timerDone = true;
        this._timerRunning = false;
        clearInterval(this._timerInterval);
        this._timerChip.setAttribute('data-done', '');
        this._timerChip.removeAttribute('data-warn');
        this._timerBeep();
      }
    }
    ...
    _timerBeep() {
      try {
        ...
        beep(0, 880); beep(0.45, 880); beep(0.9, 1174);
        setTimeout(() => ctx.close(), 2000);
      } catch (e) { /* audio unavailable — the pulsing chip is the signal */ }
    }
    ...
    _onKey(e) {
      ...
      if (e.key === 'Escape' && this._timerPanel && this._timerPanel.hasAttribute('data-open')) {
        this._timerTogglePanel(false);
        e.preventDefault();
        return;
      }
