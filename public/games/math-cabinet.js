...
function createMathCabinet({slug,read,onReceipt}) {
  let finalMessage=null,timer=null;
  ...
  window.addEventListener('message',event=>{
    ...
    if(api.pendingCoin&&m.requestId===api.pendingCoin.requestId){
      if(m.type==='arcade:coinAccepted'&&typeof m.playId==='string'){
        ...
        const {cb}=api.pendingCoin;api.pendingCoin=null;status('Could not start a ranked run. Try again or select practice.');bar.style.top="8px";bar.style.bottom="auto";cb({ok:false});
      }
    }
