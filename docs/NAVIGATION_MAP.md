...
- Admin: `/admin/textbook` (Content library) shows per-chapter status and uploads PDFs browser→Storage via signed URLs from `/api/textbook/admin`.
- Chapter list & titles: `src/data/textbook.ts`. Section index (titles, book pages, Concept Check questions, Think and Solve numbers): `src/data/textbook-sections.ts` (generated from the PDFs).
- **Homework reading** = `reading` block (student-facing card at the end of a lesson: sections + pages, link to `/textbook?ch=N`, Concept Check self-check, optional Think and Solve). Two per day, gated by `visibilityTrack`: `cpa` (one section on concept days) / `honors` (all sections + 2–3 problems). Source of truth: `scripts/textbook-reading-map-source.py` → `src/data/textbook-reading-map.json`; `python3 scripts/apply-reading-map.py` rewrites the day JSONs + teacher-plan "Hewitt reading" lines and emits `src/data/apply-reading-map.sql` for the live rows.
