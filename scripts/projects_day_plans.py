...
W01 = [
 ...
 dict(n=3, date="Wednesday, Sep 16", title="A number out of a graph · slope and area",
  ...
  segments=[
   ...
   ("25–50", "RECORD",
    "First GEWA: a cart at 0.5 m at t = 1.0 s and 2.5 m at t = 5.0 s — velocity from the slope. Given, equation, work, answer. The equation picker offers four; choosing the wrong one is diagnostic, not fatal.",
  ...
  checks=[
   ...
   "The subtraction is where this day is actually lost: 2.5 − 0.5 over 5.0 − 1.0. Watch for 2.5/5.0. That is the misconception below, showing up as arithmetic.",
  ...
  misconception=(
   ...
   "Draw two lines on the same axes: one starting at 0 and climbing steeply to 2 m in 2 s, one starting at 3 m and creeping to 3.5 m in the same 2 s. Ask which is faster. The high one is slower and looks bigger, and the room will split — that split is the lesson. Then hand it to the pairs with the press: <em>“Which number on the graph did you use, and where is the other one?”</em> Make them point at both the rise and the run before anyone says an answer.",
  )),
...
W02 = [
 dict(n=1, date="Monday, Sep 28", title="Three angles, one cart",
  ...
  checks=[
   ...
   "Acceleration, not velocity, in the second column. If the number is around 0.5–1.5 m/s² you are probably looking at acceleration; if it is the cart's speed at the gate, the whole week's fit will be wrong. Check units on the walk-around, per table.",
 ...
 dict(n=2, date="Tuesday, Sep 29", title="What would it read standing straight up?",
  targets=["pp.w02.g-diluted", "pp.w02.a-vs-angle"],
  prep=[
 ...
 dict(n=3, date="Wednesday, Sep 30", title="Off the end · run-out on the floor",
  ...
  prep=[
    ...
    ("Check", "The floor. Carpet, tile and the strip in front of the door are three different surfaces and the cart knows it. Run one lane per surface yourself before the block and keep every group on the same one."),
    ("At the front", "A cart with felt on the bottom, still. Today it earns its keep."),
  ],
  segments=[
   ("0–25", "HANDS",
    "Release, and let it run until it stops. Three times at one angle, on the taped lane. Nothing written for the first two — watch how much the three differ. The spread is data about the measurement, and this room needs to meet that idea before it needs to explain it.",
 ...
 dict(n=4, date="Thursday, Oct 1", title="The test · a new angle, locked",
  ...
  prep=[
    ...
    ("Floor", "Same lane, same surface, same tape as yesterday. If the lane moves, the prediction was never testable."),
  ],
  segments=[
   ("0–25", "PREDICT (locked)",
    "No hands first today — this is the one exception in the year and it is worth naming as one. The new angle goes up; each student reads a prediction off their own graph, writes the number, writes the reason, and writes what would prove them wrong. Then it locks.",
    "<code>w02d4-predict</code> (sentence frame, <code>gate: true</code>, tiered EN/ES)", "Evaluate", "Self-reported grades · d=1.33"),
   ("25–50", "HANDS",
    "One release at the new angle, three runs, measured on the same lane. Do it as a whole room if you can — every group watching every cart makes the spread visible and stops the quiet re-writing of predictions.",
