...
W01 = [
 dict(n=1, date="Monday, Sep 14", title="Walk the graph",
  ...
  prep=[
    ("On each table (7 groups of 3, one bridge each — 8 detectors covers it with a spare)",
...
W02 = [
 dict(n=1, date="Monday, Sep 28", title="Three angles, one cart",
  ...
  prep=[
    ("On each table", "Track and cart, a stack of books or a lab jack to set the angle, protractor (or a phone angle app — check it before the block), <strong>motion detector clamped at the low end</strong>, laptop collecting. Photogate and picket fence are the alternative, not the default — see below."),
    ("Why the detector and not the photogate", "Both give an acceleration, but they give it differently. A photogate hands the number over; the detector makes them <em>take the slope of a v–t graph to get it</em> — which is exactly what Week 1 spent five days teaching. Using the detector here turns last week's skill into this week's instrument instead of retiring it after one week. Reach for the photogate only if a detector will not see the cart on a steep ramp."),
  ...
  segments=[
   ("0–25", "HANDS",
    ...
    "<code>w02d1</code> (day callout) · <code>w02d1-faster</code> (choice: just faster, or gaining speed faster?)", "Remember / Understand", "Prior achievement · d=0.41"),
   ("25–55", "RECORD",
    "Three angles, three runs each, acceleration from the <em>slope of the detector's v–t graph</em> — their own Week 1 move, on real data. Numbers go in the table as they come, and each group plots its own three points on the angle–acceleration axes by hand before any software draws anything.",
    "<code>w02d1-data</code> (data table, 3 rows, plot enabled) · <code>w02d1-plot</code> (hand plot, before any screen draws it)", "Apply", "Worked examples · d=0.57"),
   ("55–80", "TALK",
 ...
 dict(n=2, date="Tuesday, Sep 29", title="What would it read standing straight up?",
  ...
  prep=[
    ("On each table", "Photogate clamped <strong>vertically</strong> in a stand, one picket fence, and a cushion or foam block on the floor beneath it — the fence is the one piece of gear this week a hard landing will chip. Plus yesterday's three points, on the app and on paper."),
    ("Loaded and tested", "<code>picket-fence-g</code> in the sim library — used <em>after</em> their own drops, to repeat the measurement quickly and to give a clean trace for the board. English-only; bridge at the screen."),
    ("Why the photogate here and the detector yesterday", "Not availability — physics. A free fall through a photogate is over in about half a second, far too fast for a motion detector to resolve; a picket fence cutting a beam is the right instrument and the only one that will give them a trustworthy 9.8. Yesterday the detector was right because the point was taking a slope. Each instrument earns its day."),
  ...
  segments=[
   ("0–25", "HANDS",
    "<strong>They measure g themselves.</strong> Photogate vertical, picket fence dropped through it by hand onto the cushion, three drops per table, the software reporting the acceleration. It comes out near 9.8 m/s². Then <code>picket-fence-g</code> to repeat it fast and clean. Then the question the whole block turns on: our steepest ramp read about 2 m/s². Where does the other 8 go?",
    "<code>w02d2</code> (day callout) · <code>w02d2-drop</code> (data table: 3 drops, measured g) · <code>w02d2-sim</code> (picket-fence-g)", "Apply / Understand", "Prior achievement · d=0.41"),
   ("25–55", "RECORD",
   ...
   ("55–80", "TALK",
    ...
    "<code>w02d2-beat</code> (<code>gate: true</code> — can a cart beat a falling rock?) · <code>w02d2-talk</code>", "Evaluate", "Classroom discussion · d=0.82"),
   ("80–105", "WRITE",
  ...
  checks=[
   ...
   "Their measured g lands somewhere near 9.8. A number far off usually means the gate is not truly vertical or the fence is being guided rather than dropped — fix it at the table, because this measurement is the anchor of the entire block and a bad one hands the argument to the straight line.",
  ...
  misconception=(
   ...
   "Do not tell them the relationship is a sine. Put the two numbers side by side — their extended-line prediction and the g <em>they measured with their own hands twenty minutes ago</em> — and ask the question that costs nothing to ask: “Can a cart on a ramp beat a rock falling straight down?” Wait. Then the second question, which is the actual target: “So what was wrong — the measurement, or the line?” Press for reasoning; do not resolve it with a formula. A student who ends the block saying “our line only works where we measured” has met the target more completely than one who can write a = g·sinθ.<br><br><strong>This is why the drop is real and not a sim.</strong> A student can shrug off a number a computer produced — the whole block collapses into “well, the program says 9.8.” They cannot shrug off one they made themselves with a photogate and a piece of plastic. The argument only has teeth if the measurement they are asked to trust over their own straight line is a measurement they took.<br><br><strong>If a group gets there — and one might — the resolution is theirs to find:</strong> plot acceleration against <em>sin</em>θ instead of against θ and the points really do fall on a straight line, and that line really does extrapolate to about 9.8 at sinθ = 1. Do not hand this out; it is the answer to the day's question and it is worth more discovered than told. But have it ready, because a group that asks “what if the line is the wrong shape?” has earned it.",
  )),
 ...
 dict(n=3, date="Wednesday, Sep 30", title="Off the end · run-out on the floor",
  ...
  segments=[
   ...
   ("80–105", "WRITE",
    ...
    "<code>w02d3-claim</code> (observation: what happened to the run-out, then name the friction AND where it acted)", "Understand / Analyze", "Vocabulary programs · d=0.62"),
 ...
 dict(n=5, date="Friday, Oct 2", title="The reasoning artifact · name the friction",
  ...
  segments=[
   ...
   ("100–110", "RATE",
    "Paper strip, then screen. Each day now closes with a rating on the targets <em>that day</em> captured, so today rates the friction gap and the dilution — the other two were rated on the days they were earned. Keep the wall strip showing all four so the week still reads as a whole. Read the off-week question aloud in both languages and give it out on paper. Nothing is due; one question travels home.",
    "<code>w02d5-rate</code> (self-assessment on the day's targets)", "Evaluate", "Self-reported grades · d=1.33"),
  ],
  spanish=[
