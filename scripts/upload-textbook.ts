 ...
 *
 * Usage:
 *   npm run textbook:upload -- "/Users/craigantocci/Desktop/Current/Conc. Text teachers copy"
 *
 * Expects files named cpteNN.pdf (NN = chapter) and stores them as chNN.pdf.
...
async function main() {
  const files = fs.readdirSync(folder).filter((f) => /^cpte\d{2}\.pdf$/i.test(f)).sort()
  if (!files.length) { console.error('❌ No cpteNN.pdf files found'); process.exit(1) }

  let ok = 0
  for (const f of files) {
    const n = Number(f.slice(4, 6))
