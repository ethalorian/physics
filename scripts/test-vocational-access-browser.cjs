...
;(async () => {
  ...
  await go('/admin/lesson-access?class=trades');
  await page.getByRole('heading',{name:'Trades measurement'}).waitFor();
  await page.getByText('All classes and lessons',{exact:true}).click();
  assert.equal(await page.getByLabel('Lesson program').inputValue(),'selected');
  assert.equal(await page.locator('tbody tr').count(),1);
  assert.match(await page.locator('tbody').innerText(),/Trades measurement/);
  await page.getByRole('button',{name:'Open for this class',exact:true}).click();
  await page.getByText('This class: open',{exact:true}).waitFor();
  await page.getByRole('button',{name:'Close for this class',exact:true}).click();
  await page.getByText('This class: closed',{exact:true}).waitFor();
  assert.equal(writes.at(-1).path,'/api/classes/trades/windows');
  await page.getByLabel('Lesson program').selectOption('all');
  assert.equal(await page.locator('tbody tr').count(),4);
  writes.length=0;
  console.log('PASS Engineering class shows its Trades lessons; open/close affects only that class; all-program override works.');
