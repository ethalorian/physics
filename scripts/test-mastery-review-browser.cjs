...
(async()=>{
  ...
  await page.keyboard.press('Shift+ArrowRight');await page.getByRole('heading',{name:'Student Two',exact:true}).waitFor();await page.getByText('Loading student work…',{exact:true}).waitFor({state:'hidden'});await feedback.fill('Separate student draft.');await page.keyboard.press('Escape');await page.keyboard.press('Shift+ArrowLeft');await page.getByRole('heading',{name:'Student One',exact:true}).waitFor();
  ...
  await dialog.getByRole('button',{name:'Close lesson mastery review'}).click();await page.getByRole('button',{name:'Grade 1 pending',exact:true}).click();await feedback.waitFor();await page.getByText('Loading student work…',{exact:true}).waitFor({state:'hidden'});await page.waitForFunction(()=>document.querySelector('textarea')?.value==='Strength: You connect slope to velocity.');assert.equal(await feedback.inputValue(),'Strength: You connect slope to velocity.');
  const bar=await page.getByRole('contentinfo',{name:'Active student'}).boundingBox();assert(bar.y>900 && bar.y+bar.height<=1051,'student bar is at the bottom');await page.screenshot({path:out+'/desktop.png'});await page.setViewportSize({width:390,height:844});await page.screenshot({path:out+'/mobile.png'});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.setViewportSize({width:1600,height:1050});
